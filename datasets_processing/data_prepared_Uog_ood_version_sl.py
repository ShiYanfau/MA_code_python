...
def load_uog20_samples(
    ...
    sample_ids = np.arange(specs.shape[0], dtype=np.int64)
    ...
    # UoG .mat stores each sample as (time, freq).
    # Models in this project use (freq, time).
    ...
    return X, y, subject_ids, sample_ids
...
def split_id_ood_subjects(subject_ids, test_ood_subjects=2):
    ...
    id_subjects = unique_subjects[:-test_ood_subjects]
    ood_subjects = unique_subjects[-test_ood_subjects:]

    return id_subjects, ood_subjects


def split_id_indices_by_subject_and_class(
    subject_ids,
    labels,
    id_subjects,
    train_ratio=0.7,
    ...
    for subject_id in id_subjects:
        ...
        subject_labels = np.unique(labels[subject_mask])

        for class_id in subject_labels:
            indices = np.where(subject_mask & (labels == class_id))[0]
...
def sliding_window_samples(X, y, frame_length=300, stride=100, num_classes=6):
    ...
    sample-level class label as one-hot.
    """
...
def save_split_group(h5_file, group_name, X_windows, y_windows, sample_ids):
    ...
    group.create_dataset("sample_ids", data=sample_ids, compression="gzip")
...
def prepare_uog20_subject_ood_h5(
    ...
    X, y, subject_ids, sample_ids = load_uog20_samples(
        input_dir=input_dir,
    ...
    id_indices = split_id_indices_by_subject_and_class(
    ...
    for split_name in ("train", "val", "test_id", "test_ood"):
        ...
        sample_ids_split = sample_ids[indices]
        ...
        window_sample_ids = np.repeat(sample_ids_split, windows_per_sample)
    ...
    for split_name in ("train", "val", "test_id", "test_ood"):
        ...
        print(f"{split_name}/sample_ids:", split["sample_ids"].shape)

    print("\nExample one-hot train label:")
    print(prepared_splits["train"]["labels"][0])
    print("sum:", prepared_splits["train"]["labels"][0].sum())

    print("\nExample train sample_id:")
    print(prepared_splits["train"]["sample_ids"][0])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_h5",
        default="datasets/uog20_subject_ood_sample_ids.h5",
    )
...
# 带sample id的数据集划分
# python datasets_processing/data_prepared_Uog_ood_version_sl.py --output_h5 datasets/uog20_subject_ood_sl.h5
